...
function addStatistics() {
    var statHeaders =
        ...
        '<th class="stats-col-stat">\u901a\u8fc7\u7387</th>';
    var statTable =
        '<h2>\u6d4b\u8bd5\u7edf\u8ba1</h2>' +
        '<table class="statistics" id="total-stats"><thead><tr>' +
        '<th class="stats-col-name">\u57fa\u672c\u7edf\u8ba1</th>' + statHeaders +
        '</tr></thead></table>' +
...
function renderStatTable(tableName, stats) {
    ...
    for (var i = 0, len = stats.length; i < len; i++) {
        if(tableName=='total'){
            $.tmpl(template, stats[len-1], {index: len-1}).appendTo(tbody);
            break;
        } else {
...
$.template('statColumnsTemplate',
    ...
    '<td class="stats-col-graph">' +
      ...
      '{{/if}}'+
    '</td>'+
    '<td class="stats-col-elapsed">${passPercent}%</td>'
);
